    ...
    const scenes = [
        {
            ...
            subtitle: "An Immersive Technology Conference",
        },
        {
            id: 2,
            duration: 4000,
                ...
                {currentScene === 0 && (
                    <div className="absolute inset-0 flex items-center justify-center story-scene-enter">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-br from-background via-background to-primary/5">
                            ...
                            />
                            {/* Tech glow effects */}
                            <div className="absolute top-1/4 left-[15%] w-64 h-64 bg-primary/20 rounded-full blur-3xl animate-pulse" />
                            <div
                                className="absolute top-1/4 right-[15%] w-64 h-64 bg-primary/20 rounded-full blur-3xl animate-pulse"
                                style={{ animationDelay: "0.5s" }}
                            />
                            {/* Animated circuit lines */}
                            <div className="absolute left-0 top-1/2 w-full h-0.5 bg-gradient-to-r from-transparent via-primary/30 to-transparent animate-pulse" />
                            <div className="absolute top-0 left-1/2 w-0.5 h-full bg-gradient-to-b from-transparent via-primary/30 to-transparent animate-pulse" style={{ animationDelay: "0.3s" }} />
                        </div>

                        {/* Animated TECH FLUENCE Text Background */}
                        <div className="absolute inset-0 overflow-hidden pointer-events-none">
                            {Array.from({ length: 30 }).map((_, i) => {
                                const words = ["TECH", "FLUENCE", "6", "INNOVATION", "CONFERENCE"];
                                const word = words[i % words.length];
                                return (
                                    <div
                                        key={i}
                                        className="absolute font-decorative text-2xl md:text-4xl lg:text-6xl text-primary/5 font-bold whitespace-nowrap"
                                        style={{
                                            left: `${-10 + Math.random() * 120}%`,
                                            top: `${Math.random() * 100}%`,
                                            animation: `fadeInOut ${2 + Math.random() * 2}s ease-in-out infinite`,
                        ...
                        <div className="relative z-10 text-center px-4 max-w-4xl">
                            ...
                            <div className="mb-8 flex justify-center">
                                <div className="relative">
                                    <svg viewBox="0 0 100 100" className="w-24 h-24 md:w-32 md:h-32 animate-float-slow">
                                        <defs>
                                        ...
                                        {/* Code brackets */}
                                        <path d="M30 20 L20 50 L30 80" stroke="url(#techGradient)" strokeWidth="6" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                                        <path d="M70 20 L80 50 L70 80" stroke="url(#techGradient)" strokeWidth="6" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                                        <circle cx="50" cy="50" r="8" fill="url(#techGradient)" />
                                        <path d="M40 40 L60 60 M60 40 L40 60" stroke="#ffffff" strokeWidth="3" strokeLinecap="round" />
                                    </svg>
                                    <Sparkles className="absolute -top-2 -right-4 w-8 h-8 text-primary animate-sparkle" />
                                    <Sparkles
                                        className="absolute -bottom-2 -left-4 w-6 h-6 text-primary/60 animate-sparkle"
                                        style={{ animationDelay: "0.5s" }}
                                    />
                                </div>
                            </div>
                            ...
                            {/* Main Title */}
                            <h1 className="font-decorative text-4xl md:text-6xl lg:text-8xl tech-text-gradient mb-6">
                                {scenes[0].title}
                            </h1>
                            <p
                                className={`font-cinzel text-xl md:text-2xl lg:text-3xl text-muted-foreground tracking-widest mb-8 transition-all duration-1000 ${textVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
                                    }`}
                            ...
                            {/* Event Details */}
                            <div
                                className={`space-y-3 transition-all duration-1000 ${textVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
                                    }`}
                                style={{ transitionDelay: "600ms" }}
                            >
                                <p className="font-cinzel text-base md:text-lg text-primary/90">Industry Leaders • Developers • Startup Founders</p>
                                <p className="font-cinzel text-base md:text-lg text-primary/90">High-Impact Talks • Workshops • Networking</p>
                                <p className="font-cinzel text-sm md:text-base text-muted-foreground mt-4">SDMA Auditorium, Lovely Professional University</p>
                            </div>
                        </div>
                        ...
                        <div className="absolute inset-0 pointer-events-none overflow-hidden">
                            {Array.from({ length: 20 }).map((_, i) => (
                                <div
                                    key={i}
                                    className="absolute text-primary/30 font-mono text-xs md:text-sm animate-float"
                                    style={{
